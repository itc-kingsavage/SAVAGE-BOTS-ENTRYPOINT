 ...
 * UPDATED: Manual-only Pairing Codes + QR Auto-regeneration + BOT SELECTION + DEBUG
 */
...
class SavageScanner {
    ...
    setupSocketEvents() {
        ...
        // NEW: Bot selection response
        this.socket.on('bot_selected', (data) => this.handleBotSelected(data));
    ...
    /**
     * 🆕 Handle bot selection response from server
     */
    handleBotSelected(data) {
        console.log('🤖 Bot selection response:', data);
        ...
        if (data.success && data.botName === this.selectedBot) {
            ...
            this.showNotification(`❌ Failed to select ${data.botName || 'bot'}: ${data.error || 'Unknown error'}`, 'error');
        }
    }
    ...
    handleClientLogout() {
        ...
        this.updatePairingCodeControls(false);
        this.highlightSelectedBot(null);
    ...
    /**
     * 🤖 NEW: Select bot for connection - UPDATED WITH DEBUG
     */
    selectBot(botName) {
        console.log('🤖 [DEBUG] selectBot called with:', botName);
        console.log('🤖 [DEBUG] Socket connected?', this.socket?.connected);
        console.log('🤖 [DEBUG] Socket ID:', this.socket?.id);
        console.log('🤖 [DEBUG] Is authenticated?', this.isAuthenticated);
        
        if (!this.socket || !this.socket.connected) {
            console.error('❌ [DEBUG] Socket not connected or not authenticated');
            this.showNotification('❌ Not connected to server. Please authenticate first.', 'error');
            return;
        }
        ...
        console.log(`🤖 [DEBUG] Selecting bot: ${botName}`);
        ...
        console.log('🤖 [DEBUG] Emitting select_bot event...');
        ...
        // Log for debugging
        console.log(`🤖 [DEBUG] Bot selection sent: ${botName}`);
    }
